...
export class ArtistSearchComponent implements AfterViewInit, OnDestroy {
  ...
  artistSearchbarInputFormControl: UntypedFormControl =
  ...
  @ViewChild("artistSearchResults")
  artistSearchResults: ArtistSearchResultsComponent;
    ...
    this._spotifyService.spotifyAccessTokenGrantedChanged.subscribe((value) => {
      if (!value) {
        ...
        this.artistSearchbarInputFormControl.valueChanges
          ...
          .subscribe((artistSearchbarInputValue) => {
  ...
  initialiseArtistSearch(): void {
    this.artistSearchResults$ = this.artistSearchResultsSubject.pipe(
      switchMap((artistSearchTerm) =>
        this._spotifyService
          .getArtists(artistSearchTerm)
          .pipe(catchError((error) => this.onArtistSearchError(error)))
      )
    );
  }
